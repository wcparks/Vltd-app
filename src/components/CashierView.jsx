...
export default function CashierView({ eventId, onLogout }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      ...
      list.sort((a, b) => (b.deliveredAt || b.time?.seconds || 0) - (a.deliveredAt || a.time?.seconds || 0));
      setTickets(list);
    });
    return unsub;
  }, [eventId]);
  ...
  async function markDelivered(ticket, e) {
    e.stopPropagation();
    await updateDoc(doc(db, 'tickets', ticket.id), { status: 'delivered', deliveredAt: Date.now() });
  }
      ...
      <div style={styles.list}>
        {filtered.map(ticket => {
          ...
          return (
            <div
              key={ticket.id}
              style={{ ...styles.ticketRow, ...(ticket.status === 'retrieving' ? styles.ticketRetrieving : paid ? styles.ticketPaid : {}) }}
...
const styles = {
  ...
  btnDeliver: { background: '#1a1a1a', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' },
